...
void A_star_Print(Node* p)
{
	...
	*/
	nodeNum++;
	//printf("No. %d, COST: %d, DEPTH: %d", nodeNum, p->stateNode.value, DEPTH);
	//printf("\n");
}
	...
	*/
	if(QUEUE->stateNode.manhattan == 0){
		...
		if(x1 > 0 && QUEUE->stateNode.status[x1-1][y1] != 0 && QUEUE->stateNode.status[x1-1][y1] != 7){
			...
			if(searchNode(ROOT, p) == 0 && p->stateNode.value <= DEPTH){
				insertNode(p);
			}
			else if(p->stateNode.value > DEPTH){
				//printf("Exceed depth %d\n", p->stateNode.value);
				NEXT_DEPTH = (NEXT_DEPTH == 0 || p->stateNode.value < NEXT_DEPTH) ? p->stateNode.value : NEXT_DEPTH;
				free(p);
			}
			else{
				free(p);
...
int main()
{
	...
	Node* START = new Node;
	START->nextState = NULL;
	START->stateNode = QUEUE->stateNode;
	//A_star_Search();
	/*
	printf("\n");
	Node* GOAL = new Node;
	...
	*/
	DEPTH = QUEUE->stateNode.value;
	while(DONE == 0 && QUEUE != NULL){
		//printf("DEPTH: %d\n", DEPTH);
		while(QUEUE != NULL && DONE == 0){
			A_star_Search();
		}
		DEPTH = NEXT_DEPTH;
		QUEUE = new Node;
		QUEUE->nextState = NULL;
		QUEUE->stateNode = START->stateNode;
		//printf("NEXT_DEPTH: %d\n", NEXT_DEPTH);
		NEXT_DEPTH = 0;
		ROOT = NULL;
		//system("pause");
